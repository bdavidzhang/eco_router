#!/usr/bin/env bash
# run_10min_v5.sh — Autonomous research run with Live TUI dashboard
#
# Self-bootstrapping: creates venv, installs deps with pip, then runs the
# workbench with sensor_logger_v4.sh (67 channels) capturing hardware data.
#
# New in v5:
#   - Live TUI dashboard (Rich Live) — real-time GPU stats, progress bar,
#     Pareto frontier, updated every 2s during experiments
#   - All v4 sensor channels preserved (67 channels)
#
# Based on v4:
#   - 67 sensor channels (was 45): GPU throttle+idle+brake, PCIe gen/width, PSI pressure,
#     NVMe IO counters, VM page faults, memory controller util, SM clocks,
#     CPU time decomposition, memory decomposition, network bandwidth
#   - sensor_logger_v4.sh lives in-repo (no fragile ../../ path)
#   - Runs analyze_runs.py on exit for cross-run Pareto analysis
#
# Usage:
#   ./run_10min_v4.sh                              # defaults
#   ./run_10min_v4.sh --minutes 5 --budget 30      # quick run
#   ./run_10min_v4.sh --region eu_france            # French nuclear grid
#   SENSOR_INTERVAL=2 ./run_10min_v4.sh             # 2s sensor sampling
#
...
cat << 'BANNER'
  ...
  ╔══════════════════════════════════════════════════════════════╗
  ║  🔬 Auto-Improving LLM Research Workbench v5               ║
  ║     Live TUI dashboard · 67-channel sensor logging          ║
  ╚══════════════════════════════════════════════════════════════╝
...
# NOTE: Do NOT pipe through | tee — that kills Rich Live TUI!
# Instead, --log-file writes directly to the log file from Python.
# NOTE: No --total-time flag — we rely on --max-iter to run all experiments.
